...
void FPANE_Log(FREContext context, NSString* message) {
    ...
    AirAACPlayer_FPANE_DispatchEventWithInfo(context, @"LOGGING", message);
}
...
NSArray* AirAACPlayer_FPANE_FREObjectToNSArrayOfUIImage(FREObject object) {
    ...
    for (NSInteger i = 0; i < arrayLength; i++) {
        ...
        UIImage *image = AirAACPlayer_FPANE_FREBitmapDataToUIImage(itemRaw);
...
FREObject AirAACPlayer_FPANE_CreateError(NSString* error, NSInteger* id) {
    ...
    FREObject argV[] = {
            AirAACPlayer_FPANE_NSStringToFREObject(error),
            freId
    };
